...
class Profile(View):
    ...
    def get(self,request):
        ...
        if Staff.objects.all().filter(name=userhere).exists():
            ...
            if Hod.objects.all().filter(name=userhere):
                flag1=1
                return render(request,self.template_name,{'ob':ob,'u':u,'user':userhere,'li':li,'flag':flag,'ub':ub,'flag1':flag1})
            return render(request,self.template_name,{'ob':ob,'u':u,'user':userhere,'li':li,'flag':flag})
    ...
    def post(self,request):
        ...
        if Hod.objects.all().filter(name=userhere):
            flag1 = 1 
            return render(request,self.template_name,{'ob':ob,'u':u,'user':userhere,'flag1':flag1})
